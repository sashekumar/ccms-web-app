-- ============================================================================
-- CCMS ACL Seed Data: Deferment Monitoring Module
-- ============================================================================
-- Module: DEFERMENT_MON | order=102 | icon=hourglass-half | route=/deferment-monitoring
-- Actions: VIEW (read), UPDATE (update status/resolve)
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Deferment Monitoring Module';
PRINT '============================================================================';
PRINT '';
...
IF EXISTS (SELECT 1 FROM ccms_acl_modules WHERE module_code = 'DEFERMENT_MON')
BEGIN
    SELECT @ModuleId = module_id FROM ccms_acl_modules WHERE module_code = 'DEFERMENT_MON';
    PRINT '  ℹ Module already exists: DEFERMENT_MON (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
ELSE
BEGIN
    ...
    VALUES (
        'Deferment Monitoring', 'DEFERMENT_MON', NULL, 'hourglass-half', '/deferment-monitoring', 102, 1, GETDATE(), 'admin'
    );

    SELECT @ModuleId = SCOPE_IDENTITY();
    PRINT '  ✓ Created module: DEFERMENT_MON (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
...
BEGIN
    ...
    VALUES (@ModuleId, @ActionView, 'View Deferred Cases', 1, GETDATE(), 'admin');
...
BEGIN
    INSERT INTO ccms_acl_module_actions (module_id, action_id, action_label, is_active, created_at, created_by)
    VALUES (@ModuleId, @ActionUpdate, 'Update Deferment Status', 1, GETDATE(), 'admin');
...
PRINT 'Actions linked to DEFERMENT_MON: ' + CAST(@ActionsCount AS VARCHAR);
PRINT 'Permissions granted to Super Admin: ' + CAST(@PermissionsCount AS VARCHAR);
PRINT '';
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Deferment Monitoring Module - Completed Successfully';
PRINT '============================================================================';
