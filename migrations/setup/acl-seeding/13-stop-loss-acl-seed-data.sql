-- ============================================================================
-- CCMS ACL Seed Data: Stop Loss Module
-- ============================================================================
-- Purpose: Sets up all access control configurations for the Stop Loss module
-- Module: STOP_LOSS
-- Route: /stop-loss
-- Tables: ccms_stop_loss_data
--
-- Actions: 2 permissions
--   - VIEW: View stop loss records and statistics
--   - UPDATE: Create and update stop loss records
--
-- Business Rules:
--   - Stop Loss Dashboard provides a consolidated view of all stop loss records
--   - Officers with VIEW permission can see all records and statistics
--   - Officers with UPDATE permission can create new records and update amounts/flags
--
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Stop Loss Module';
PRINT '============================================================================';
PRINT '';
...
IF EXISTS (SELECT 1 FROM ccms_acl_modules WHERE module_code = 'STOP_LOSS')
BEGIN
    SELECT @ModuleId = module_id FROM ccms_acl_modules WHERE module_code = 'STOP_LOSS';
    PRINT '  ℹ Module already exists: STOP_LOSS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
ELSE
BEGIN
    ...
    VALUES (
        'Stop Loss Management',     -- module_name
        'STOP_LOSS',                -- module_code
        NULL,                       -- category_id (NULL = uncategorized, top-level menu)
        'chart-line',               -- icon (Font Awesome icon name: fas fa-chart-line)
        '/stop-loss',               -- route
        70,                         -- display_order (after Financials=65)
        1,                          -- is_active
        GETDATE(),                  -- created_at
        'admin'                     -- created_by
    );
    ...
    PRINT '  ✓ Created module: STOP_LOSS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
...
BEGIN
    ...
    VALUES (@ModuleId, @ActionView, 'View Stop Loss Records', 1, GETDATE(), 'admin');
...
BEGIN
    INSERT INTO ccms_acl_module_actions (module_id, action_id, action_label, is_active, created_at, created_by)
    VALUES (@ModuleId, @ActionUpdate, 'Manage Stop Loss Records', 1, GETDATE(), 'admin');
...
PRINT 'Actions linked to STOP_LOSS module: ' + CAST(@ModuleActionCount AS VARCHAR);
...
BEGIN
    PRINT '✓✓✓ SUCCESS: Stop Loss module ACL setup complete! ✓✓✓';
    PRINT '';
    PRINT 'Module Details:';
    PRINT '  - Module Code: STOP_LOSS';
    PRINT '  - Display Name: Stop Loss Management';
    PRINT '  - Route: /stop-loss';
    PRINT '  - Icon: chart-line';
    PRINT '  - Actions: 2 (VIEW, UPDATE)';
    PRINT '  - Super Admin: All 2 permissions granted';
    PRINT '';
    PRINT 'Permission Summary:';
    PRINT '  - VIEW: View all stop loss records, history and statistics';
    PRINT '  - UPDATE: Create new records, update premiums and history flag';
    PRINT '';
