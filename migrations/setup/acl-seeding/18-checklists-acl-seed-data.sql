-- ============================================================================
-- CCMS ACL Seed Data: Checklists Module
-- ============================================================================
-- Purpose: Sets up all access control configurations for the Checklists module
-- Module: CHECKLISTS
-- Route: /checklists
-- Table: ccms_checklists
--
-- Actions: 2 permissions
--   - VIEW: View checklists and statistics
--   - UPDATE: Create and update checklist entries
--
-- Business Rules:
--   - Checklists Dashboard displays all checklist items and completion status
--   - Officers with VIEW permission can see all checklist records per claim
--   - Officers with UPDATE permission can create/modify/delete checklist entries
--   - Statistics dashboard shows total checklists, types breakdown, top modifiers
--
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Checklists Module';
PRINT '============================================================================';
PRINT '';
...
IF EXISTS (SELECT 1 FROM ccms_acl_modules WHERE module_code = 'CHECKLISTS')
BEGIN
    SELECT @ModuleId = module_id FROM ccms_acl_modules WHERE module_code = 'CHECKLISTS';
    PRINT '  ℹ Module already exists: CHECKLISTS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
ELSE
BEGIN
    ...
    VALUES (
        'Checklists',                  -- module_name
        'CHECKLISTS',                  -- module_code
        NULL,                          -- category_id (NULL = uncategorized, top-level menu)
        'tasks',                       -- icon (Font Awesome icon name: fas fa-tasks)
        '/checklists',                 -- route
        95,                            -- display_order (after Notifications=90)
        1,                             -- is_active
        GETDATE(),                     -- created_at
        'admin'                        -- created_by
    );
    ...
    PRINT '  ✓ Created module: CHECKLISTS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
...
BEGIN
    ...
    VALUES (@ModuleId, @ActionView, 'View Checklist Records', 1, GETDATE(), 'admin');
...
BEGIN
    INSERT INTO ccms_acl_module_actions (module_id, action_id, action_label, is_active, created_at, created_by)
    VALUES (@ModuleId, @ActionUpdate, 'Manage Checklist Records', 1, GETDATE(), 'admin');
...
PRINT 'Actions linked to CHECKLISTS module: ' + CAST(@ModuleActionCount AS VARCHAR);
...
PRINT '';
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Checklists Module - Completed Successfully';
PRINT '============================================================================';
