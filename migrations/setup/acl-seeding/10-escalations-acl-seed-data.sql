-- ============================================================================
-- CCMS ACL Seed Data: Escalations Module
-- ============================================================================
-- Purpose: Sets up all access control configurations for the Escalations module
-- Module: ESCALATIONS
-- Route: /escalations
-- Tables: ccms_escalations, ccms_escalation_updates, ccms_escalation_sources, ccms_escalation_natures
-- 
-- Actions: 2 permissions
--   - VIEW: View escalations, assigned escalations, escalation history, and details
--   - UPDATE: Manage escalations, assign, update status, priority, and add updates
-- 
-- Business Rules:
--   - Escalations dashboard provides a consolidated view of all escalations
--   - Officers with VIEW permission can see all escalations
--   - Officers with UPDATE permission can assign, manage, and close escalations
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Escalations Module';
PRINT '============================================================================';
PRINT '';
...
IF EXISTS (SELECT 1 FROM ccms_acl_modules WHERE module_code = 'ESCALATIONS')
BEGIN
    SELECT @ModuleId = module_id FROM ccms_acl_modules WHERE module_code = 'ESCALATIONS';
    PRINT '  ℹ Module already exists: ESCALATIONS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
ELSE
BEGIN
    ...
    VALUES (
        'Escalations Management',   -- module_name
        'ESCALATIONS',              -- module_code
        NULL,                       -- category_id (NULL = uncategorized, top-level menu)
        'exclamation-triangle',     -- icon (FontAwesome icon name)
        '/escalations',             -- route
        55,                         -- display_order (after Monitoring=50)
        1,                          -- is_active
        GETDATE(),                  -- created_at
        'admin'                     -- created_by
    );
    ...
    PRINT '  ✓ Created module: ESCALATIONS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
...
BEGIN
    ...
    VALUES (@ModuleId, @ActionView, 'View Escalations', 1, GETDATE(), 'admin');
...
BEGIN
    INSERT INTO ccms_acl_module_actions (module_id, action_id, action_label, is_active, created_at, created_by)
    VALUES (@ModuleId, @ActionUpdate, 'Manage Escalations', 1, GETDATE(), 'admin');
...
PRINT 'Actions linked to ESCALATIONS module: ' + CAST(@ModuleActionCount AS VARCHAR);
...
BEGIN
    PRINT '✓✓✓ SUCCESS: Escalations module ACL setup complete! ✓✓✓';
    PRINT '';
    PRINT 'Module Details:';
    PRINT '  - Module Code: ESCALATIONS';
    PRINT '  - Display Name: Escalations Management';
    PRINT '  - Route: /escalations';
    PRINT '  - Icon: exclamation-triangle';
    PRINT '  - Actions: 2 (VIEW, UPDATE)';
    PRINT '  - Super Admin: All 2 permissions granted';
    PRINT '';
    PRINT 'Permission Summary:';
    PRINT '  - VIEW: View all escalations, assigned escalations, and escalation history';
    PRINT '  - UPDATE: Manage escalations, assign, update status/priority, and add updates';
    PRINT '';
