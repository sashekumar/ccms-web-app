-- ============================================================================
-- CCMS ACL Seed Data: Financials Module (Payment Advice)
-- ============================================================================
-- Purpose: Sets up all access control configurations for the Financials module
-- Module: FINANCIALS
-- Route: /financials
-- Tables: ccms_payment_advice, ccms_pa_line_items, ccms_pa_summary,
--         ccms_pa_consultation_breakdown, ccms_pa_uncovered_charges,
--         ccms_pa_payments, ccms_multi_payment_advice,
--         ccms_multi_payment_advice_details
--
-- Actions: 2 permissions
--   - VIEW: View payment advices, detail, statistics
--   - UPDATE: Create and update payment advices
--
-- Business Rules:
--   - Finance Dashboard provides a consolidated view of all payment advices
--   - Officers with VIEW permission can see all PA records and statistics
--   - Officers with UPDATE permission can create new PAs and update status/amounts
--
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '============================================================================';
PRINT 'CCMS ACL Setup: Financials Module (Payment Advice)';
PRINT '============================================================================';
PRINT '';
...
IF EXISTS (SELECT 1 FROM ccms_acl_modules WHERE module_code = 'FINANCIALS')
BEGIN
    SELECT @ModuleId = module_id FROM ccms_acl_modules WHERE module_code = 'FINANCIALS';
    PRINT '  ℹ Module already exists: FINANCIALS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
ELSE
BEGIN
    ...
    VALUES (
        'Financial Management',     -- module_name
        'FINANCIALS',               -- module_code
        NULL,                       -- category_id (NULL = uncategorized, top-level menu)
        'dollar-sign',              -- icon (Font Awesome icon name: fas fa-dollar-sign)
        '/financials',              -- route
        65,                         -- display_order (after Investigations=60)
        1,                          -- is_active
        GETDATE(),                  -- created_at
        'admin'                     -- created_by
    );
    ...
    PRINT '  ✓ Created module: FINANCIALS (ID: ' + CAST(@ModuleId AS VARCHAR) + ')';
END
...
BEGIN
    ...
    VALUES (@ModuleId, @ActionView, 'View Payment Advices', 1, GETDATE(), 'admin');
...
BEGIN
    INSERT INTO ccms_acl_module_actions (module_id, action_id, action_label, is_active, created_at, created_by)
    VALUES (@ModuleId, @ActionUpdate, 'Manage Payment Advices', 1, GETDATE(), 'admin');
...
PRINT 'Actions linked to FINANCIALS module: ' + CAST(@ModuleActionCount AS VARCHAR);
...
BEGIN
    PRINT '✓✓✓ SUCCESS: Financials module ACL setup complete! ✓✓✓';
    PRINT '';
    PRINT 'Module Details:';
    PRINT '  - Module Code: FINANCIALS';
    PRINT '  - Display Name: Financial Management';
    PRINT '  - Route: /financials';
    PRINT '  - Icon: currency-dollar';
    PRINT '  - Actions: 2 (VIEW, UPDATE)';
    PRINT '  - Super Admin: All 2 permissions granted';
    PRINT '';
    PRINT 'Permission Summary:';
    PRINT '  - VIEW: View all payment advices, statistics, and sub-entity details';
    PRINT '  - UPDATE: Create new payment advices and update status, amounts, tracking fields';
    PRINT '';
