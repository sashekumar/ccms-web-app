...
-- Drop views
IF OBJECT_ID('vw_user_permissions', 'V') IS NOT NULL DROP VIEW vw_user_permissions;
IF OBJECT_ID('vw_role_permissions', 'V') IS NOT NULL DROP VIEW vw_role_permissions;
GO

-- Drop tables (in reverse order of dependencies)
IF OBJECT_ID('ccms_user_roles', 'U') IS NOT NULL DROP TABLE ccms_user_roles;
IF OBJECT_ID('ccms_role_permissions', 'U') IS NOT NULL DROP TABLE ccms_role_permissions;
IF OBJECT_ID('ccms_module_actions', 'U') IS NOT NULL DROP TABLE ccms_module_actions;
IF OBJECT_ID('ccms_actions', 'U') IS NOT NULL DROP TABLE ccms_actions;
IF OBJECT_ID('ccms_modules', 'U') IS NOT NULL DROP TABLE ccms_modules;
IF OBJECT_ID('ccms_categories', 'U') IS NOT NULL DROP TABLE ccms_categories;
IF OBJECT_ID('ccms_roles', 'U') IS NOT NULL DROP TABLE ccms_roles;
GO
...
CREATE NONCLUSTERED INDEX idx_ccms_roles_code ON ccms_roles(role_code);
CREATE NONCLUSTERED INDEX idx_ccms_roles_active ON ccms_roles(is_active);
GO
...
CREATE NONCLUSTERED INDEX idx_ccms_categories_code ON ccms_categories(category_code);
CREATE NONCLUSTERED INDEX idx_ccms_categories_active ON ccms_categories(is_active);
CREATE NONCLUSTERED INDEX idx_ccms_categories_order ON ccms_categories(display_order);
GO
...
CREATE TABLE ccms_modules (
    ...
    CONSTRAINT fk_ccms_modules_category FOREIGN KEY (category_id) 
        REFERENCES ccms_categories(category_id)
);

CREATE NONCLUSTERED INDEX idx_ccms_modules_code ON ccms_modules(module_code);
CREATE NONCLUSTERED INDEX idx_ccms_modules_active ON ccms_modules(is_active);
CREATE NONCLUSTERED INDEX idx_ccms_modules_category ON ccms_modules(category_id);
GO
...
CREATE NONCLUSTERED INDEX idx_ccms_actions_code ON ccms_actions(action_code);
CREATE NONCLUSTERED INDEX idx_ccms_actions_active ON ccms_actions(is_active);
GO

-- ============================================================================
-- 5. MODULE ACTIONS TABLE (Bridge Table)
...
-- ============================================================================
CREATE TABLE ccms_module_actions (
    ...
    CONSTRAINT fk_ccms_module_actions_module FOREIGN KEY (module_id) 
        REFERENCES ccms_modules(module_id) ON DELETE CASCADE,
    CONSTRAINT fk_ccms_module_actions_action FOREIGN KEY (action_id) 
        REFERENCES ccms_actions(action_id) ON DELETE CASCADE,
    CONSTRAINT uq_ccms_module_actions UNIQUE (module_id, action_id)  -- One action per module
);

CREATE NONCLUSTERED INDEX idx_ccms_module_actions_module ON ccms_module_actions(module_id);
CREATE NONCLUSTERED INDEX idx_ccms_module_actions_action ON ccms_module_actions(action_id);
CREATE NONCLUSTERED INDEX idx_ccms_module_actions_active ON ccms_module_actions(is_active);
GO

-- ============================================================================
-- 6. ROLE PERMISSIONS TABLE (Bridge Table)
...
-- ============================================================================
CREATE TABLE ccms_role_permissions (
    ...
    CONSTRAINT fk_ccms_role_permissions_role FOREIGN KEY (role_id) 
        REFERENCES ccms_roles(role_id) ON DELETE CASCADE,
    CONSTRAINT fk_ccms_role_permissions_module_action FOREIGN KEY (module_action_id) 
        REFERENCES ccms_module_actions(module_action_id) ON DELETE CASCADE,
    CONSTRAINT uq_ccms_role_permissions UNIQUE (role_id, module_action_id)  -- One permission per role-action
);

CREATE NONCLUSTERED INDEX idx_ccms_role_permissions_role ON ccms_role_permissions(role_id);
CREATE NONCLUSTERED INDEX idx_ccms_role_permissions_module_action ON ccms_role_permissions(module_action_id);
CREATE NONCLUSTERED INDEX idx_ccms_role_permissions_granted ON ccms_role_permissions(granted);
GO

-- ============================================================================
-- 7. USER ROLES TABLE (Bridge Table)
...
CREATE TABLE ccms_user_roles (
    ...
    CONSTRAINT fk_ccms_user_roles_role FOREIGN KEY (role_id) 
        REFERENCES ccms_roles(role_id) ON DELETE CASCADE,
    CONSTRAINT uq_ccms_user_roles UNIQUE (user_id, role_id)  -- One assignment per user-role pair
);

CREATE NONCLUSTERED INDEX idx_ccms_user_roles_user ON ccms_user_roles(user_id);
CREATE NONCLUSTERED INDEX idx_ccms_user_roles_role ON ccms_user_roles(role_id);
CREATE NONCLUSTERED INDEX idx_ccms_user_roles_active ON ccms_user_roles(is_active);
CREATE NONCLUSTERED INDEX idx_ccms_user_roles_expires ON ccms_user_roles(expires_at);
GO

-- ============================================================================
-- Add Foreign Key to ccms_users
-- ============================================================================
ALTER TABLE ccms_user_roles
ADD CONSTRAINT fk_ccms_user_roles_user 
...
-- Seed Roles
-- ============================================================================
SET IDENTITY_INSERT ccms_roles ON;
INSERT INTO ccms_roles (role_id, role_name, role_code, description, is_system_role, is_active) VALUES
(1, 'Super Admin', 'SUPER_ADMIN', 'Full system access with all permissions including user management', 1, 1);
SET IDENTITY_INSERT ccms_roles OFF;
GO

-- ============================================================================
-- Seed Categories
-- ============================================================================
SET IDENTITY_INSERT ccms_categories ON;
INSERT INTO ccms_categories (category_id, category_name, category_code, description, icon, display_order, is_active) VALUES
(1, 'System Administration', 'SYSTEM_ADMIN', 'System administration and permission control', 'cog', 100, 1);
SET IDENTITY_INSERT ccms_categories OFF;
GO

-- ============================================================================
-- Seed Modules
-- ============================================================================
INSERT INTO ccms_modules (module_name, module_code, description, icon, route, display_order, category_id) VALUES
...
-- Seed Actions
-- ============================================================================
INSERT INTO ccms_actions (action_name, action_code, description) VALUES
...
DECLARE @dashboardModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'DASHBOARD');
DECLARE @userMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'USER_MANAGEMENT');
DECLARE @roleMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'ROLE_MANAGEMENT');
DECLARE @moduleMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'MODULE_MANAGEMENT');
DECLARE @actionMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'ACTION_MANAGEMENT');
DECLARE @moduleActionMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'MODULE_ACTION_MANAGEMENT');
DECLARE @rolePermMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'ROLE_PERMISSION_MANAGEMENT');
DECLARE @userRoleAssignModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'USER_ROLE_ASSIGNMENT');
DECLARE @categoryMgmtModuleId BIGINT = (SELECT module_id FROM ccms_modules WHERE module_code = 'CATEGORY_MANAGEMENT');
DECLARE @viewActionId BIGINT = (SELECT action_id FROM ccms_actions WHERE action_code = 'VIEW');
DECLARE @createActionId BIGINT = (SELECT action_id FROM ccms_actions WHERE action_code = 'CREATE');
DECLARE @updateActionId BIGINT = (SELECT action_id FROM ccms_actions WHERE action_code = 'UPDATE');
DECLARE @deleteActionId BIGINT = (SELECT action_id FROM ccms_actions WHERE action_code = 'DELETE');

INSERT INTO ccms_module_actions (module_id, action_id, action_label) VALUES
...
FROM ccms_users u
INNER JOIN ccms_user_roles ur ON u.user_id = ur.user_id
INNER JOIN ccms_roles r ON ur.role_id = r.role_id
INNER JOIN ccms_role_permissions rp ON r.role_id = rp.role_id
INNER JOIN ccms_module_actions ma ON rp.module_action_id = ma.module_action_id
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM ccms_user_roles WHERE user_id = @user_id AND role_id = @role_id)
        BEGIN
            UPDATE ccms_user_roles
            SET is_active = 1,
                assigned_at = GETDATE(),
        ...
        ELSE
        BEGIN
            INSERT INTO ccms_user_roles (user_id, role_id, assigned_by, expires_at)
...
PRINT '  1. ccms_categories (Module categories)';
PRINT '  2. ccms_roles (System roles)';
PRINT '  3. ccms_modules (Application modules with category FK)';
PRINT '  4. ccms_actions (CRUD actions)';
PRINT '  5. ccms_module_actions(Module-action mappings)';
PRINT '  6. ccms_role_permissions (Role-permission assignments)';
PRINT '  7. ccms_user_roles (User-role assignments)';
PRINT '';
PRINT 'VIEWS CREATED:';
PRINT '  1. vw_user_permissions (Full permission details with categories)';
PRINT '  2. vw_role_permissions (Role permission summary)';
PRINT '';
...
PRINT '  2. Verify: SELECT * FROM ccms_categories;';
PRINT '  3. Verify: SELECT * FROM ccms_modules ORDER BY category_id, display_order;';
